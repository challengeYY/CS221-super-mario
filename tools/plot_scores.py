...
def smoothByAverage(scores, factor):
    ...
    for i in range(len(scores)):
        newScores.append(float(sum(scores[max(0, i - factor+1): i+1])) / (min(factor, i+1)))
    return newScores
...
def main():
    ...
    parser.add_argument('--smooth_factor', dest='smooth_factor', action='store', default=50,
